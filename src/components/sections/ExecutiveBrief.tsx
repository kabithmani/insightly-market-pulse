...
export default function ExecutiveBrief({ city }: Props) {
  const themes = executiveThemes.filter(t => t.city === city);
      ...
      <div className="rounded-3xl bg-card shadow-card p-6 md:p-8">
        ...
        <div className="prose prose-sm max-w-none text-muted-foreground">
          <p className="text-foreground font-medium leading-relaxed">
            {city === "Bangalore" && "The Bangalore residential market enters March 2026 in its strongest cycle since 2019. RERA registrations are up 23% YOY, luxury segment sales have surged 41%, and NRI pre-sales have crossed the 38-47% mark at premium launches. North Bangalore has emerged as the defining thesis of this cycle, with ₹580Cr+ deployed by top developers in just 15 days."}
            {city === "Pune" && "Pune's western IT corridor is reaching a pricing inflection point. Hinjewadi leads with a CMI of 8.7, while satellite markets Tathawade (+22%) and Wagholi (+24%) show breakout appreciation. Metro Phase 3 tunneling at 40% completion is the primary catalyst. RERA filings up 15% YOY signal sustained developer confidence."}
            {city === "Mumbai" && "Mumbai's dual-catalyst story — Coastal Road and NMIA — is creating two distinct investment corridors. BKC rents hit all-time highs, GIC's ₹4,200Cr acquisition validates ultra-premium, while Ulwe's 18% YOY appreciation leads the growth corridor. Institutional capital inflows are at record levels."}
          </p>
        </div>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mt-6">
          {metrics.slice(0, 4).map(m => (
            <div key={m.metric} className="rounded-2xl bg-muted/50 p-4">
              ...
              <p className={`text-xs mt-1 font-semibold ${m.signal === "STRONG" ? "text-green-600" : m.signal === "RISING" ? "text-blue-600" : "text-amber-600"}`}>
                {m.change}
              </p>
            </div>
          ))}
        ...
        <div className="space-y-4">
          {themes.map((t, i) => (
            <div key={i} className="flex gap-4 items-start p-4 rounded-2xl bg-muted/30 hover:bg-muted/50 transition-colors">
              <span className="shrink-0 w-8 h-8 rounded-xl gradient-primary flex items-center justify-center text-primary-foreground font-bold text-sm">
                {i + 1}
              </span>
              <div>
